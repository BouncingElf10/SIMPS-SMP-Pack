...
def update_pack_description(directory, latest_version):
    pack_mcmeta_path = os.path.join(directory, "SimpsPack", "pack.mcmeta")
    # Update the desc with latest commit version
    try:
        with open(pack_mcmeta_path, 'r') as file:
            pack_data = json.load(file)
        
        # Update the description with the latest commit version
        description = pack_data['pack']['description']
        updated_description = (
            f"\u00a78Maintained by: \u00a72BouncingElf\u00a78, "
            f"\u00a7bPerry\u00a78, & \u00a71Player1O\u00a78.\u00a76 v{latest_version}"
        )
        pack_data['pack']['description'] = updated_description

        # Write the updated data back to the pack.mcmeta file
        with open(pack_mcmeta_path, 'w') as file:
            json.dump(pack_data, file, indent=2)
        
        print("pack.mcmeta file updated successfully.")
    except Exception as e:
        print(f"Error updating pack.mcmeta file: {e}")
...
def extract_first_commit_message(git_folder):
    try:
        ...
        result = subprocess.run(['git', 'log', '--format=%s', '-1'], stdout=subprocess.PIPE, cwd=git_folder)
...
def create_hello_file():
    ...
    if directory:
        ...
        # Fetch latest commit message and extract version
        latest_commit_message = get_latest_commit_message(repo_url)
        latest_version = extract_version(latest_commit_message)
        
        # Update the desc with latest commit version
        update_pack_description(directory, latest_version)
